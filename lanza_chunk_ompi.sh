...
do
    ...
    do
	mpirun -np $i --map-by node -mca btl openib --mca btl_openib_allow_ib true --oversubscribe \
       --hostfile myhostfile.$$ --mca  mpi_warn_on_fork 0 ./${exe} 0 0 0 0 1 $p > ${dir}/iallreduce_${i}_procs_${p}_chunk.dat
	mpirun -np $i --map-by node -mca btl openib --mca btl_openib_allow_ib true --oversubscribe \
         --mca coll libnbc,basic --mca coll_libnbc_iallreduce_algorithm 1  \
       --hostfile myhostfile.$$ --mca  mpi_warn_on_fork 0 ./${exe} 0 0 0 0 1 $p > ${dir}/iallreduce_${i}_procs_${p}_chunk_ring.dat
    done
done
