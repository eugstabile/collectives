...
for i in ${procs}
do
    mpirun -n $i --map-by node -mca btl openib --mca btl_openib_allow_ib true --oversubscribe   --hostfile myhostfile.$$ --mca  mpi_warn_on_fork 0 ./${exe} 0 0 1 > ${dir}/${NODELIST}_${i}.dat
    #for a in 1 2 3 4 5 6
    #do 
    #    mpirun -n $i --map-by node -mca btl openib --mca btl_openib_allow_ib true --oversubscribe --mca coll_tuned_use_dynamic_rules 1 --mca coll_tuned_allreduce_algorithm $a  --hostfile myhostfile --mca  mpi_warn_on_fork 0 ./${exe} 0 0 1 > ${dir}/allreduce_${a}_alg_${i}.dat
    #done
    #for p in ${part}
    #do
