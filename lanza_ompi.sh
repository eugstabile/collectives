...
do
    mpirun -n $i --map-by node --oversubscribe  \
        ${net} \
       --hostfile myhostfile.$$ --mca  mpi_warn_on_fork 0 ./${exe} 1 0 0 > ${dir}/redscat_auto_${i}.dat
    for a in ${algs}
    do 
        mpirun -n $i --map-by node --oversubscribe  \
        ${net} \
        --mca coll_tuned_use_dynamic_rules 1 --mca coll_tuned_allgather_algorithm $a \
        --hostfile myhostfile.$$ --mca  mpi_warn_on_fork 0 ./${exe} 1 0 0 > ${dir}/bcast_${a}_alg_${i}.dat
    done
    #for p in ${part}
    #do
    #	mpirun -n $i --map-by node -mca btl openib --mca btl_openib_allow_ib true --oversubscribe   --hostfile myhostfile --mca  mpi_warn_on_fork 0 ./${exe} 1 $p 0 > ${dir}/bcast_${i}_procs_${p}_parts.dat
    #done
done
