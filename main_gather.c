...
double original_gather(TYPE * in, TYPE * out, TYPE * sol, size_t s, int wsize,int rank, int reps, MPI_Comm  comm){
    START_TEST;
    MPI_Gather(in, s, MPI_TYPE, out, s, MPI_TYPE, ROOT, comm );
    END_TEST;
}

//DYNAMIC for testing
double half_igather(TYPE * in, TYPE * out, TYPE * sol, size_t s, int wsize,int rank, int reps, MPI_Comm  comm, int halfs){
    if(s < halfs){halfs=1;}
    ...
    for(h=0;h<halfs-1;h++)
    {
        ...
        MPI_Igather( &in[h*half_size], half_size, MPI_TYPE, &out[h*half_sizeo], half_size, MPI_TYPE, ROOT, comm, &request[h] );
        sent+=half_size;
    }
    h=halfs-1;
    //printf("Proc %d enviando %d de %d envios con tamaño %d a partir de posicion: %d y almaceno en posicion %d\n", rank, h, halfs, s-sent, h*half_size, h*half_sizeo); fflush(stdout);
    MPI_Igather( &in[h*half_size], s-sent, MPI_TYPE, &out[h*half_sizeo], s-sent, MPI_TYPE, ROOT, comm, &request[h] );
    MPI_Waitall(halfs,request,status);
    END_TEST;
}


double chunk_igather(TYPE * in, TYPE * out, TYPE * sol, size_t s, int wsize,int rank, int reps, MPI_Comm  comm, int chunk){
...
int main(int argc, char *argv[])
{
    ...
    size_t max_count = 1024*1024*1024; // 256 MB per proc for send
    ...
    if(rank == 0){
        ...
        printf("#SIZE(bytes)\t");
        if(ori == 1){
            printf("Gather(%d)\t",wsize);
        }
    
        if(opt == 1){
            printf("%d_half_ig(%d)\t",part,wsize);
        }
        if(chunk == 1){
            printf("%d_chunk_ig(%d)\t",chunksize,wsize);
        }
    printf("\n");
    }
    ...
    for (s=ss; s<=count; s*=2){
        ...
        wsize = total_wsize;
        if(ori == 1){   
            double time_bcast = original_gather(in,out,sol,s,wsize,rank,reps,MPI_COMM_WORLD);
            if(rank == 0){
              printf("%lf,%lf\t", time_bcast, ((((double) s*sizeof(TYPE))/1000000)/time_bcast));
            }
        }
        if( opt == 1){ 
	    double time_4hiallreduce = half_igather(in,out,sol,s,wsize,rank,reps,MPI_COMM_WORLD,part);
            if(rank == 0){
                printf("%lf,%lf\t", time_4hiallreduce, ((((double) s*sizeof(TYPE))/1000000)/time_4hiallreduce));
            } 
        }
        if ( chunk == 1){
	    double t_chunk = chunk_igather(in,out,sol,s,wsize,rank,reps,MPI_COMM_WORLD,chunksize);       
            if(rank == 0){
